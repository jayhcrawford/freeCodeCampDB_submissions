...
echo "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games ON teams.team_id = games.winner_id WHERE game_id IS NOT NULL ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT DISTINCT year, name FROM teams FULL JOIN games ON teams.team_id = games.winner_id WHERE game_id IS NOT NULL AND round='Final';")"
